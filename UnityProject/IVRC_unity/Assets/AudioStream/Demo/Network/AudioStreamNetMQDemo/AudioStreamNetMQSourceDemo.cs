...
namespace AudioStreamDemo
{
    ...
    public class AudioStreamNetMQSourceDemo : MonoBehaviour
    {
        ...
        FMOD_SystemW.INPUT_DEVICE_INFO defaultInput; // just for UI display 
        IEnumerator Start()
        {
        ...
        void OnGUI()
        {
            ...
            GUILayout.Label(string.Format("<b>(the source of the audio is netradio @: {0}, it plays independently)</b>", this.audioSource.GetComponent<AudioStream.AudioStream>().url), AudioStreamSupport.UX.guiStyleLabelNormal);
